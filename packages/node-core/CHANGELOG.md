...
## [Unreleased]
### Changed
- The multichain rewind lock no longer waits forever on a chain that cannot release it, or on a notification that never arrives. The `_global` lock table is now the source of truth: every status change (a notification, a 30s poll, a listener reconnect, startup) goes through one idempotent `reconcile()` that reads this chain's row, so a notification lost to a dead listener connection is recovered at the next poll, and a handler failure (for example an RPC error during the header search) is logged and retried instead of blocking every later notification. A release that is not yet committed is not mistaken for a new, earlier lock. The listener connection is kept alive with a 30s heartbeat and replaced when it errors, ends or stops answering. Only chains whose node registered for the lock at startup (the new `multiChainRewindLock` metadata key, written `true` by participating nodes and `false` by nodes started with `--disable-multichain-rewind-lock`) are enrolled in a rewind; a node on an image without the lock or a metadata table left behind by a removed chain is skipped and logged. A chain with no new blocks runs its pending rewind from the fetch loop instead of only after the next processed block. The wait log names the chains still to complete and is written every 30s rather than every 3s. The reindex skip path rolls its transaction back when releasing the lock fails.
...
- With `--allow-schema-migration`, the startup schema sync diffs the previously applied schema (persisted in the new `appliedSchemaSDL` metadata key) against the current one and applies additive changes to existing entities in place (add column/table/index/enum value/relation) instead of only creating missing tables, so adding a field no longer requires renaming the entity and orphaning its data. The baseline is written in the same transaction as the DDL. Destructive changes (dropping or retyping a field, dropping an entity) drop the column and its data, so they are refused unless `SUBQL_ALLOW_DESTRUCTIVE_MIGRATION=true`. The first boot after enabling the flag only records the current schema as the baseline; later edits migrate against it.
