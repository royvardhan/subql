...
## [Unreleased]
### Changed
- The multichain rewind lock no longer waits forever on a chain that cannot release it. Only chains whose node registered for the lock at startup (the new `multiChainRewindLock` metadata key, written `true` by participating nodes and `false` by nodes started with `--disable-multichain-rewind-lock`) are enrolled in a rewind; a node on an image without the lock or a metadata table left behind by a removed chain is skipped and logged. Every chain re-reads the `_global` lock table every few seconds while indexing (in addition to the LISTEN notifications), so a notification lost to a dropped listener connection or a restart no longer leaves the chain in "waiting for other chains" after the lock is released, and the wait log names the chains still to complete. A chain with no new blocks runs its pending rewind from the fetch loop instead of only after the next processed block. The listener connection is re-registered when it errors or ends.
