...
## [Unreleased]
### Changed
- Multichain rewind lock liveness (see node-core): the lock table is polled and reconciled rather than relying on notifications alone, the listener connection is kept alive and replaced when it dies, only nodes registered for the lock are enrolled in a rewind, and idle chains run their pending rewind without waiting for a block.
