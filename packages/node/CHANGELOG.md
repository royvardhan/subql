...
## [Unreleased]
### Changed
- Multichain rewind lock liveness (see node-core): only nodes registered for the lock are enrolled in a rewind, status is re-read from the lock table rather than relying on notifications alone, idle chains run their pending rewind without waiting for a block, and the listener connection is re-registered when lost.
